...
def join_event(event_id, user_id, time_start, time_end, duty):
    ...
    for e in user.event_doing['event']:
        if (datetime.strptime(e['time_start'], "%Y-%m-%d %H:%M:%S") <= time_start and datetime.strptime(e['time_end'], "%Y-%m-%d %H:%M:%S") >= time_start) \
        or (datetime.strptime(e['time_start'], "%Y-%m-%d %H:%M:%S") <= time_end and datetime.strptime(e['time_end'], "%Y-%m-%d %H:%M:%S") >= time_end):
            return False
